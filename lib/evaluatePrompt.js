...
//
// Grades an optimized prompt (V1) against the user's intent and the
...
//     Together AI client (which is dedicated to GLM optimization/refinement).
// ---------------------------------------------------------------------------
...
// Latency control. The rubric is an explicit 0-10 schema with anchors, so the
// judge doesn't need deep deliberation: measured 15.3s at the default effort
// vs 6.9s at "low" with equal-or-better scores on the same prompt.
const JUDGE_REASONING_EFFORT = "low";

// Hard ceiling for a judge call. The judge runs ~7s; 30s catches a hung
...
Schema:
{
  "faithfulness": number,      // 0-10 INTENT FIDELITY: does the prompt faithfully capture the user's original intent, including user-supplied text, data, drafts, notes, and examples, without inventing or dropping requirements?
...
- If the user supplied material to summarize, rewrite, analyze, or use, the prompt must carry it verbatim as delimited data. Replacing it with a placeholder or dropping it is a genuine faithfulness defect: score faithfulness below 7 and suggest restoring the supplied material. Placeholders are appropriate only for inputs the user did not provide.
...
/**
 * Evaluate an optimized prompt with the GPT-5-mini judge.
 *
 ...
 * @param {string}   input.optimizedPromptV1 - The V1 output to grade.
