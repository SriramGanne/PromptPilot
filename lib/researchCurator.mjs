...
import { EMBEDDING_DIM as EMBED_DIM } from "./models.mjs";
...
//
// CALIBRATION (data-driven, not a guess): measured with text-embedding-3-small
// (1024-dim) over the 66 pairwise similarities among the 12 seed entries — all
// KNOWN-DISTINCT techniques: max 0.644 (CoT ↔ ToT), p95 0.578, median 0.376.
// A light paraphrase of an entry scores ~0.93 against the original. 0.85 sits
// clearly above the distinct-pair ceiling and below the paraphrase band, so
// only a genuine restatement is skipped. The runtime semantic CACHE uses 0.82
// for a different job (matching paraphrased queries).
...
// ---------------------------------------------------------------------------
// Embedding (shared lib/embeddings.mjs path, with retry/backoff)
// ---------------------------------------------------------------------------
 ...
 * @param {number}  [opts.simThreshold=0.85]      - semantic-dedup cosine cutoff
...
export async function runCuration({
  ...
  for (const cand of candidates) {
    try {
      ...
      const embedding = await embedDocument(cand.content ?? cand.abstract);
