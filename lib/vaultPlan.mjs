 ...
 */
export function buildPlan({ existing = [], additions = [], referenceOnly = [], liveRows = [] }) {
  const errors = [];
  const warnings = [];
  const actions = [];
  let unchanged = 0;
  ...
  for (const rec of existing) {
    ...
    validateSet({ ...live, ...set }, { isInsert: false, label, errors });

    if (live.title !== rec.current_title && live.title !== set.title) {
  ...
  for (const rec of insertRecs) {
    ...
    if (matches.length) {
      ...
      if (matches.length !== 1 || previouslyClaimed) {
        ...
        const differences = changedFields(matches[0], set);
        if (differences.length) {
